...
// Start MongoDB driver installation
...
const verifyJWT = ( req, res, next) => {
  ...
  if (!authorization){
    return res.status(410).send({error:true, message: 'unauthorized access'});
  }
  const token = authorization.split(' ')[1];
  console.log(('Token got') ,token);
  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, ( error, decoded) =>{
    if(error){
      return res.status(403).send({ error: true, message: 'unauthorized accessed' });
    }
    req.decoded = decoded
    next()
  })
...
async function run() {
  try {
    ...
    app.get("/bookings", verifyJWT, async (req, res) => {
      ...
      console.log('Cone back after verification', decoded);
...
// End of MongoDB connection from driver
...
app.listen(port, () => {
  console.log(`Doctor server listening on: ${port}`);
});
